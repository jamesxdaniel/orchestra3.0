<?php defined('BASEPATH') OR exit('No direct script access allowed');
    ...
    class Userdbcontext extends CI_Model {
        ...
        public function check_user($useremail, $password) {
            ...
            if ($query->num_rows() > 0) {
                // echo json_encode($query->row()->user_password);
                // echo json_encode($password);
                // echo json_encode(sha1($password));
        ...
        public function view_profile($id) {
            $query = $this->db
                    ->select('users.*,ut.team_name as main_team_name,ust.team_name as sub_team_name,users_team_members.date_assigned,users_team_members.team_id')
                    ->from($this->table_name)
                    ->join('users_team_members','users_team_members.user_id = users.user_id')
        ...
        public function show_teammates($team_id) {
            $query = $this->db
                    ->select('users.user_status,users.user_photo,users.user_id,users.user_full_name,users.user_level,ut.team_name as main_team_name,ust.team_name as sub_team_name,users_team_members.date_assigned,users_team_members.team_id')
                    ->from($this->table_name)
                    ->join('users_team_members','users_team_members.user_id = users.user_id')
                    ->join('users_sub_team ust', 'ust.id = users_team_members.sub_team_id')
                    ->join('users_team ut','ut.team_id = users_team_members.team_id')
